...
6. **Given** một user mới nhận được invitation email, **When** click vào "Activation Link" trong email để xác thực, **Then** hệ thống yêu cầu thiết lập mật khẩu, ghi nhận xác thực email thành công và tự động login 1 lần vào tenant với session policy giống login thường.
...
Super Admin xem dashboard toàn hệ thống: thống kê per-tenant (số user, số khóa, usage), so sánh tenants. Super Admin gán plan cho tenant, tạo và gán Custom Plan (override limit + feature entitlement), theo dõi quota, và hệ thống cảnh báo khi tenant gần chạm giới hạn.
...
**Independent Test**: Super Admin mở dashboard hệ thống, xem usage per-tenant, tạo Custom Plan mới, gán vào tenant, và xác nhận limit/entitlement có hiệu lực ngay; đồng thời xác nhận cảnh báo khi tenant gần hết quota.
...
6. **Given** Super Admin tạo Custom Plan "Pro-SSO-200" (user limit=200, course limit=100, bật entitlement SSO), **When** gán Custom Plan cho tenant, **Then** giới hạn mới và entitlement SSO có hiệu lực ngay, và Tenant Admin nhận thông báo thay đổi.
7. **Given** tenant đang dùng Custom Plan có bật entitlement SSO, **When** Tenant Admin cấu hình SSO và user đăng nhập SSO lần đầu, **Then** hệ thống cho phép cấu hình SSO và thực hiện JIT provisioning theo FR-044.
...
- **FR-016**: Hệ thống PHẢI hỗ trợ student tự đăng ký qua subdomain của tenant (nếu tenant cho phép) hoặc qua invitation. Với local auth, luồng Activation Link (xác thực email + thiết lập mật khẩu + auto-login 1 lần) PHẢI áp dụng cho tất cả user được tạo qua invitation với role Student, Instructor, Tenant Admin phụ, và cả Student self-register. Phiên auto-login sau kích hoạt PHẢI có cùng session policy như login thường của tenant. Activation Link PHẢI là single-use, hết hạn sau 24 giờ, và hệ thống PHẢI cho phép gửi lại link mới; khi link mới được phát hành, toàn bộ activation link cũ của user đó PHẢI bị vô hiệu ngay lập tức để chỉ link mới nhất còn hiệu lực.
...
- **FR-022**: Hệ thống PHẢI tự động ghi nhận tiến độ học viên: unit đã xem, quiz đã làm, điểm số. Với quiz có nhiều attempt hợp lệ, điểm chính thức PHẢI lấy theo điểm cao nhất.
...
**Bảo Mật & Chống Lạm Dụng (Security & Abuse Prevention)**
...
- **FR-032**: Hệ thống PHẢI hỗ trợ các plan cơ bản từ mã nguồn (Free/Trial, Pro, Enterprise).
- **FR-033**: Super Admin PHẢI có thể gán, thay đổi plan cho tenant, và tạo "Custom Plan" để override các limit (số user, số khóa học) và feature entitlement cho tenant đó. Custom Plan PHẢI có thể bật entitlement SSO/JIT độc lập với plan Enterprise. Thay đổi plan hoặc entitlement PHẢI có hiệu lực ngay.
...
- **FR-043**: Tenant thuộc plan Enterprise, hoặc tenant có Custom Plan được bật entitlement SSO, PHẢI có thể cấu hình SSO. Với tenant plan Enterprise, SLA riêng được quản lý offline qua hợp đồng; trong v1 KHÔNG áp dụng cơ chế traffic ưu tiên ở mức hệ thống.
- **FR-044**: Với tenant có entitlement SSO và đã cấu hình SSO, hệ thống PHẢI tự động tạo user qua Just-In-Time provisioning khi user đăng nhập SSO thành công lần đầu, với role mặc định là Student. Nếu email đã tồn tại trong cùng tenant và user chưa liên kết SSO, hệ thống PHẢI liên kết vào user hiện có, giữ role hiện tại, lưu mapping định danh SSO, và KHÔNG ĐƯỢC tạo user trùng. Định danh liên kết SSO PHẢI duy nhất theo bộ khóa (tenant_id, idp_issuer, idp_subject). Nếu user đã có mapping liên kết SSO hợp lệ nhưng email claim thay đổi ở lần đăng nhập sau, hệ thống PHẢI cho đăng nhập theo mapping hiện có, PHẢI cập nhật email local theo claim mới, và PHẢI ghi audit log cho thay đổi này.
...
- **Quiz**: Bài kiểm tra gắn với unit. Thuộc tính: tiêu đề, danh sách câu hỏi, loại câu hỏi, tổng điểm tối đa, điểm đạt, thời lượng làm bài (time limit), số lần làm tối đa (attempt limit), cờ xáo trộn câu hỏi, cửa sổ thời gian mở/đóng, quy tắc tính điểm chính thức (scoring policy = HighestValidAttempt). Quan hệ: thuộc Unit, có nhiều QuizAttempt.
- **QuizAttempt**: Lần làm bài quiz của student. Thuộc tính: attempt_no, trạng thái (InProgress, Submitted, AutoSubmitted, Expired), started_at, submitted_at, score, answers_snapshot_ref, is_counted_toward_attempt_limit (AutoSubmitted do hết giờ vẫn tính là attempt hợp lệ). Quan hệ: thuộc Quiz, thuộc User (Student).
...
- **Progress**: Tiến độ học của student. Thuộc tính: khóa đã hoàn thành (%), unit đã xem, kết quả quiz (tổng hợp từ QuizAttempt theo quy tắc HighestValidAttempt). Quan hệ: liên kết User với Course.
...
- **Brute force & Rate limit**: Hệ thống PHẢI có cơ chế khóa tài khoản và rate limit API public để chống brute force và spam (chi tiết chức năng tại FR-042).
...
- SSO integration (SAML, OIDC) khả dụng cho tenant plan Enterprise hoặc tenant có Custom Plan được bật entitlement SSO; các tenant không có entitlement SSO sử dụng email/password authentication.
